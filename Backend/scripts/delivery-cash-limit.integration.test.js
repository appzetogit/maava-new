...
before(async () => {
    ...
    cash = await import('../src/modules/food/delivery/services/deliveryFinance.service.js');
    walletMath = await import('../src/modules/food/delivery/services/walletMath.js');
...
test('cash in hand counts the whole order, not the rider cut', async () => {
    ...
    const held = (await cash.getCashInHandFor([RIDER])).get(String(RIDER));
...
test('the production scenario: a rider well past the limit is blocked', async () => {
    ...
    assert.equal(walletMath.isAtOrAboveCashLimit(held, limit), true, 'this is the case that silently passed before');
});
...
test('depositing restores headroom and unblocks the rider', async () => {
    ...
    assert.equal(walletMath.isAtOrAboveCashLimit((await cash.getCashInHandFor([RIDER])).get(String(RIDER)), limit), true);

    await deposit(RIDER, 1600);
    const after = (await cash.getCashInHandFor([RIDER])).get(String(RIDER));
    assert.equal(after, 0, 'a full deposit must clear the float');
    assert.equal(walletMath.isAtOrAboveCashLimit(after, limit), false, 'the rider must be able to work again');
});
...
test('a wallet top-up is the rider own money and does not clear COD owed', async (t) => {
    ...
    assert.equal(walletMath.isAtOrAboveCashLimit(held, await cash.getCashLimit()), true);
});
...
test('a pending deposit does not count until it completes', async () => {
    ...
    assert.equal((await cash.getCashInHandFor([RIDER])).get(String(RIDER)), 1000);
});

test('exactly at the limit blocks; a rupee under does not', async () => {
    const limit = 1500;
    assert.equal(walletMath.isAtOrAboveCashLimit(1499, limit), false);
    assert.equal(walletMath.isAtOrAboveCashLimit(1500, limit), true, 'at the limit must block, not just above it');
    assert.equal(walletMath.isAtOrAboveCashLimit(1501, limit), true);
});

test('a limit of zero never blocks anyone', async () => {
    await codOrder(RIDER, 999999);
    const held = (await cash.getCashInHandFor([RIDER])).get(String(RIDER));
    assert.equal(walletMath.isAtOrAboveCashLimit(held, 0), false, 'unconfigured installs must not lock every rider out');
});
...
test('over-depositing floors at zero rather than granting extra headroom', async () => {
    ...
    assert.equal((await cash.getCashInHandFor([RIDER])).get(String(RIDER)), 0);
});
...
test('an admin-set ledger value still blocks, matching what the rider is shown', async () => {
    ...
    assert.equal(walletMath.isAtOrAboveCashLimit(held, 1500), true);
});
