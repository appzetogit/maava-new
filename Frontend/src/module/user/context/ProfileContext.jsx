...
export function ProfileProvider({ children }) {
  ...
  const [vegMode, setVegMode] = useState(() => {
    ...
    // Default OFF. This used to default ON, which meant a first-time user saw
    // veg-only menus without ever asking for them -- and any restaurant whose
    // items carry a wrong or missing foodType showed a completely empty menu.
    // That is not hypothetical: there are live restaurants with every item
    // tagged Non-Veg, idli and masala dosa included, and their menus rendered
    // blank. Showing everything by default fails safe; the toggle still works
    // and anyone who has already set it keeps their choice.
  ...
  // Saved addresses come back as Mongo subdocuments with _id (no id), so every
  // match below goes through addrId; matching on addr.id silently matched nothing.
  const addrId = (a) => String(a?._id || a?.id || "")
  const addAddress = useCallback(async (address) => {
    try {
      ...
      if (newAddress) {
        setAddresses((prev) => {
          const newId = addrId(newAddress)
          const updated = [...prev.filter((a) => addrId(a) !== newId && a.label !== newAddress.label), newAddress]
  ...
  const updateAddress = useCallback(async (id, updatedAddress) => {
    try {
      ...
      if (updatedAddr) {
        setAddresses((prev) => {
          const updated = prev.map((addr) => (addrId(addr) === String(id) ? updatedAddr : addr))
          localStorage.setItem("userAddresses", JSON.stringify(updated))
          return updated
        })
        return updatedAddr
  ...
  const deleteAddress = useCallback(async (id) => {
    try {
      ...
      setAddresses((prev) => {
        const newAddresses = prev.filter((addr) => addrId(addr) !== String(id))
        if (newAddresses.length && !newAddresses.some((a) => a.isDefault)) {
          newAddresses[newAddresses.length - 1] = { ...newAddresses[newAddresses.length - 1], isDefault: true }
        }
  ...
  const setDefaultAddress = useCallback(async (id) => {
    ...
    setAddresses((prev) => {
      const updated = prev.map((addr) => ({ ...addr, isDefault: addrId(addr) === String(id) }))
      localStorage.setItem("userAddresses", JSON.stringify(updated))
      return updated
    })
    return true
  ...
  const getAddressById = useCallback((id) => {
    return addresses.find((addr) => addrId(addr) === String(id))
  }, [addresses])
