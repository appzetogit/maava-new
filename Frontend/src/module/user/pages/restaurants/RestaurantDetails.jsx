          ...
          restaurant?.menuSections && Array.isArray(restaurant.menuSections) && restaurant.menuSections.length > 0 && (
            <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 py-6 sm:py-8 md:py-10 lg:py-12 space-y-6 md:space-y-8 lg:space-y-10">
              {getFilteredSections().map(({ section, originalIndex }, sectionIndex) => {
                ...
                // Compute the post-filter list once. The two blocks below used to
                // test section.items.length -- the RAW count -- and then render the
                // FILTERED list, so a section whose items were all filtered out
                // rendered an open, completely empty container with no explanation.
                // That is what "the menu does not load" looks like from outside.
                const visibleItems = sortMenuItems(filterMenuItems(section.items || [])) || []
                const hidingNonVeg = vegMode === true || filters.vegNonVeg === "veg"
                      ...
                      <div className="text-center py-8">
                        <p className="text-gray-500 dark:text-gray-400 text-sm md:text-base">
                          {(section.items || []).length === 0
                            ? (originalIndex === 0 ? "No dish recommended" : "Nothing in this section yet")
                            : hidingNonVeg
                              ? "Veg mode is on -- nothing here is marked veg"
