      ...
      watchIdRef.current = navigator.geolocation.watchPosition(
        async (pos) => {
          try {
            ...
            // GEOCODE-GATE: reverse geocoding used to run on EVERY watchPosition
            // tick -- a Google call per tick -- and only afterwards did the code
            // check whether we had actually moved, throwing the result away if we
            // had not. Standing still therefore cost a request every few seconds
            // and logged a full geocode round trip each time. The movement check
            // is the same one that already gated the state update below (~10m); it
            // just runs before we pay for the lookup now. prevLocationCoordsRef
            // holds the last position we actually resolved, so this measures
            // distance from that, not from the previous tick.
            const GEOCODE_MIN_DELTA = 0.0001 // ~10 metres, matches coordThreshold
            const prevFix = prevLocationCoordsRef.current
            const movedEnough =
              !prevFix.latitude ||
              !prevFix.longitude ||
              Math.abs(prevFix.latitude - latitude) > GEOCODE_MIN_DELTA ||
              Math.abs(prevFix.longitude - longitude) > GEOCODE_MIN_DELTA
            if (!movedEnough) {
              return
            }
