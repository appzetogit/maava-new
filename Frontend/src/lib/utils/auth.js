...
export function isModuleAuthenticated(module) {
  ...
  if (isTokenExpired(token)) {
    if (module === "user" && localStorage.getItem("user_refreshToken")) return true;
    clearModuleAuth(module);
    return false;
  }
...
export function clearModuleAuth(module) {
  ...
  if (module === "user") localStorage.removeItem("user_refreshToken");
...
export function setAuthData(module, token, user, refreshToken = null) {
  try {
    ...
    // Without this the session could never be renewed: the first 401 or the
    // access token's expiry signed the customer out.
    if (refreshToken) localStorage.setItem(`${module}_refreshToken`, refreshToken);
